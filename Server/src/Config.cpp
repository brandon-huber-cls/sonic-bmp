...
Config::Config() {
    ...
    // Initialize access control defaults
    enable_access_control = false;
    allowed_source_ips.clear();
            ...
            for (YAML::const_iterator it = root.begin(); it != root.end(); ++it) {
                ...
                if (node.Type() == YAML::NodeType::Map) {
                    if (key.compare("base") == 0)
                        ...
                        parseMapping(node);
                    else if (key.compare("access_control") == 0)
                        parseAccessControl(node);
...
/**
 * Parse the access control configuration
 *
 * \param [in] node     Reference to the yaml NODE
 */
void Config::parseAccessControl(const YAML::Node &node) {
    if (node["enabled"]) {
        try {
            enable_access_control = node["enabled"].as<bool>();

            if (debug_general)
                std::cout << "   Config: access_control.enabled : " << enable_access_control << std::endl;

        } catch (YAML::TypedBadConversion<bool> err) {
            printWarning("access_control.enabled is not of type boolean", node["enabled"]);
        }
    }

    if (node["allowed_source_ips"] && node["allowed_source_ips"].Type() == YAML::NodeType::Sequence) {
        allowed_source_ips.clear();

        for (std::size_t i = 0; i < node["allowed_source_ips"].size(); i++) {
            std::string ip_range = node["allowed_source_ips"][i].Scalar();

            if (ip_range.size() > 0) {
                allowed_source_ips.push_back(ip_range);

                if (debug_general)
                    std::cout << "   Config: access_control.allowed_source_ips = " << ip_range << "\n";
            }
        }

        if (enable_access_control && allowed_source_ips.empty()) {
            std::cout << "WARN: Access control is enabled but no allowed source IPs are configured. All connections will be rejected." << std::endl;
        }
    }
}
